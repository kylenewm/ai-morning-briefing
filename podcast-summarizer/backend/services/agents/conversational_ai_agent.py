"""
Conversational AI Search Agent

Specialist agent for finding articles about:
- Voice AI and conversational systems
- AI agents and agent frameworks
- Real-time AI interactions
- Products like OpenAI Realtime, Gemini Live, LangGraph, Vapi, ElevenLabs
"""
import logging
from typing import Any

from .base_search_agent import BaseSearchAgent
from ..search_queries import CONVERSATIONAL_AI_QUERY
from ...test_config import AGENT_TARGET_ARTICLES

logger = logging.getLogger(__name__)


class ConversationalAIAgent(BaseSearchAgent):
    """
    Specialist agent for Conversational AI articles.
    
    Target: 3 articles (or 1 in test mode).
    """
    
    @property
    def TARGET_ARTICLES(self) -> int:
        # Use test config if set, otherwise production default
        if AGENT_TARGET_ARTICLES:
            return AGENT_TARGET_ARTICLES["conversational_ai"]
        return 3
    
    @property
    def INITIAL_QUERY(self) -> str:
        return CONVERSATIONAL_AI_QUERY
    
    @property
    def QUERY_TYPE(self) -> str:
        return "conversational_ai"


# Factory function for LangGraph Studio
def create_graph(config: Any = None):
    """
    Factory function for LangGraph Studio.
    Takes a RunnableConfig and returns the compiled graph.
    """
    agent = ConversationalAIAgent(enable_tracing=False)  # Disable duplicate tracing in Studio
    return agent.graph

